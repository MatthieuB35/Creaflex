function Output=TAC_Eureka(window,VLetter,NLetter,FontSizeEureka,ColorWord,ColorBackgroundWT,ColorBackgroundNT)
...
Screen('FillRect', window, ColorBackgroundWT);

%Ask the question to the participant if he gets a sudden response
DrawFormattedText(window, QuestionInstruction, 'center', 'center',ColorWord);
...
while ~TimeOut
    ...
    if( (keyTime - TimeStart) > Time2Wait_Q-2)
        ...
        Screen('FillRect', window,ColorBackgroundNT);
